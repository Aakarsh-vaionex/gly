...
const navigation = [
  {name:'Home',href:"/"},
  { name: 'New Release', href: '/about' },
  { name: 'Characters', href: '/dashboard' },
  { name: 'Movies', href: '/contact' },
  
  { name: 'TV Shows', href: '/contact' },
...
useEffect(() => {
  ...
  const options = ['DC','Marvel','Miscallaneous']

  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  return (
    <header className="bg-red-700">
      <nav
        className={`mx-auto  flex max-w-7xl items-center justify-between p-6 lg:px-8 ${scroll?'opacity-':'opacity-100'}`}
        aria-label="Global"
      >
